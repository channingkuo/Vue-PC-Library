<template>
    <div class="add-new-user">
        <div class="overlay-edit-user" :style="{display: showModel, width: width + 'px', height: height + 'px'}"></div>
        <div ref="edituser" class="edit-user" :style="{ top: marginTop + 'px', left: marginLeft + 'px', display: showModel }">
            <!-- header -->
            <div ref="edituserheader" class="edit-user-header">
                <div style="font-size: 14px;color: #d7d5d5;height: 16px;line-height:16px;">编辑用户</div>
            ...
            <div style="margin: 18px 38px 0px 38px;" class="add-user-content">
                <el-form label-position="left" label-width="80px" :model="formUserData">
                    <el-form-item label="用　户" required>
                        <el-input :disabled="true" v-model="formUserData.userName" placeholder="请输入内容" style="width: 100%;"></el-input>
                    </el-form-item>
                    <el-form-item label="邮　件" required>
                        <el-input :disabled="true" v-model="formUserData.userEmail" placeholder="请输入内容" style="width: 100%;"></el-input>
                    </el-form-item>
                    <el-form-item label="手　机" required>
                        <el-input :disabled="true" v-model="formUserData.userPhone" placeholder="请输入内容" style="width: 100%;"></el-input>
                    </el-form-item>
            ...
            isShow: function(val){
                ...
                if(val){
                    ...
                    this.$nextTick(function () {
                        /**************控制弹出层拖拽移动************************/
                        /**************必须写在this.$nextTick()中***************/
                        var dialog = this.$refs.edituser
                        var header = this.$refs.edituserheader
        ...
        methods: {
            loadSelectedUser (){
                ...
                that.$auth.post(apiUrl, bodyData).then((resp) => {
                    ...
                    if(resp.data.errcode === '0'){
                        that.formUserData = {
                            userName: resp.data.resData.userInfo.empname,
                            userEmail: resp.data.resData.userInfo.email,
                            userPhone: resp.data.resData.userInfo.phone,
                            userPassword: ''
                        }
                    }else{
            ...
            saveTheUser (){
                ...
                let bodyData = {
                    ...
                    phonenum: that.formUserData.userPhone,
...
<style>
    ...
    .add-user-content .el-input.is-disabled .el-input__inner{
        border-radius: 0px;
